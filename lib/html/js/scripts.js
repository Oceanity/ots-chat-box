...
$(function () {
    ...
    const regexClasses = [
        { regex: /trans\s+rights/ig, class: "trans stroke name-stroke" }
    ];
    ...
    if (channel) {
        ...
        socket.on("message", (data) => {
            ...
            ParseURLs(splitMessage, matches, isSuperUser, (message) => {
                ParseEmotes(message, emotes, (message) => {
                    ...
                    // Add special classes
                    for (let r of regexClasses) {
                        if (message.match(r.regex)) {
                            block.addClass(r.class);
                            break;
                        }
                    }
        ...
        socket.on("timeout", (res) => {
            console.log(`%c[User ${res.user} Timed Out for ${res.length}]`, "font-weight: bold; color: #d19b4b;");
        ...
        // On Ban
        socket.on("ban", (res) => {
            console.log(`%c[User ${res.user} Banned]`, "font-weight: bold; color: #e54947;");
            $(chatBox).find(`li[data-from="${res.user}"]`).remove();
            if (lastUser === res.user) {
                (lastUser = null), (lastMessage = null);
            }
